...
import Button from '../../src/button/button.windows/button';
...
describe('Button', () => {
  ...
  it('create button component with one node', () => {
    const string = renderToString(<Button><span>Hello</span></Button>);
    expect(string).to.match(/<span.*>Hello/);
  });

  it('create button component with multiple nodes', () => {
    const string = renderToString(<Button><span>Hello</span><span>World</span></Button>);
    expect(string).to.match(/<span.*>Hello/);
